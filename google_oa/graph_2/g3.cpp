...
int main() {
    ...
    while (!pq.empty()) {
        ...
        } // no need of relaxation check here, because we are allowing multiple visits to the same node, and we are only interested in the k shortest paths to the destination node n, so we will push all possible paths to the priority queue, and let the cnt array handle the limit of k visits per node.
    }
