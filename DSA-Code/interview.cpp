...
================================================================================
  ...
  08. LINKED LISTS
      - [CORE] Reverse Linked List (Iterative & Recursive)
      - [HIGH] Reverse Linked List II (Range Reversal)
      - [CORE] Reverse Nodes in K-Group
      - [CORE] Merge Two Sorted Lists
      - [CORE] Merge K Sorted Lists
      - [CORE] Sort List (Merge Sort on Linked List)
      - [HIGH] Reorder List
      - [CORE] Remove Nth Node From End of List
      - [HIGH] Intersection of Two Linked Lists
      - [CORE] Palindrome Linked List
      - [CORE] Add Two Numbers
      - [CORE] Linked List Cycle I (Detection)
      - [CORE] Linked List Cycle II (Cycle Start)
      - [CORE] Copy List with Random Pointer
...
// [CORE] Reverse Linked List (Iterative & Recursive)
// Idea: Iteratively flip next pointers using three pointers: prev, curr, nextNode.
// Time: O(n)
...
// [HIGH] Reverse Linked List II (Between Left and Right)
// Idea: Locate node prior to left, then iteratively splice subsequent nodes to the head of the subsegment.
// Time: O(n)
// Space: O(1)
...
// [CORE] Reverse Nodes in K-Group
// Idea: Check if k nodes exist. Reverse subsegment of k nodes, link to recursively/iteratively reversed remainder.
// Time: O(n)
// Space: O(1)
ListNode* reverseKGroup(ListNode* head, int k) {
    ListNode* check = head;
    for (int i = 0; i < k; ++i) {
        if (!check) return head; // Fewer than k nodes remain
        check = check->next;
    }
...
// [CORE] Merge Two Sorted Lists
// Idea: Use a dummy head to simplify pointer manipulation and merge two sorted chains in a single pass.
// Time: O(n + m)
// Space: O(1)
...
// [CORE] Merge K Sorted Lists (Min-Heap / Priority Queue)
// Idea: Min-heap of size k storing active list node pointers. Extract minimum node, append to tail, push next.
...
// [CORE] Sort List (Merge Sort on Linked List)
// Idea: Fast/slow pointers find midpoint to bisect list; recursively sort both halves and merge.
// Time: O(n log n)
...
ListNode* sortList(ListNode* head) {
    ...
    prev->next = nullptr; // Bisect list
    return mergeTwoLists(sortList(head), sortList(slow));
}

// [HIGH] Reorder List (L0 -> Ln -> L1 -> Ln-1 ...)
// Idea: 1) Find middle with fast/slow pointers; 2) Reverse second half; 3) Interleave two halves.
// Time: O(n)
// Space: O(1)
...
// [CORE] Remove Nth Node From End of List
// Idea: Fast pointer advances n + 1 steps ahead of slow pointer from dummy head. Slow pointer lands before target.
// Time: O(n)
// Space: O(1)
...
// [HIGH] Intersection of Two Linked Lists
// Idea: Two pointers switch heads upon reaching ends. Both traverse (lenA + lenB) and meet at intersection or null.
// Time: O(n + m)
// Space: O(1)
...
// [CORE] Palindrome Linked List
// Idea: Find mid with fast/slow pointers, reverse second half in-place, and compare values node by node.
// Time: O(n)
// Space: O(1)
...
// [CORE] Add Two Numbers
// Idea: Traverse both lists simultaneously, maintaining running carry and creating result nodes digit by digit.
// Time: O(max(n, m))
// Space: O(1) auxiliary (excluding output)
...
// [CORE] Linked List Cycle I (Detection)
// Idea: Floyd's cycle detection. Fast pointer advances 2 steps, slow pointer 1 step. If they meet, a cycle exists.
// Time: O(n)
// Space: O(1)
...
// [CORE] Linked List Cycle II (Find Cycle Start)
// Idea: Upon fast/slow collision, reset slow to head. Both move 1 step per turn; collision point is cycle entrance.
// Time: O(n)
// Space: O(1)
...
// [CORE] Copy List with Random Pointer
// Idea: 3-pass algorithm: 1) duplicate nodes side-by-side; 2) assign random pointers; 3) detach duplicate list.
// Time: O(n)
// Space: O(1) auxiliary
NodeWithRandom* copyRandomList(NodeWithRandom* head) {
    ...
    curr = head;
    NodeWithRandom* clonedHead = head->next;
    while (curr) {
        NodeWithRandom* copy = curr->next;
        curr->next = copy->next;
        curr = curr->next;
