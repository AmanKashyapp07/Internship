...
/*
 ...
 |-----|--------------------------------------------|-----------------------------------|----------|----------|
 | 81  | Minimum Refueling Stops                    | Max-Heap Past Gas Stations        | O(N logN)| O(N)     |
 | 82  | Trapping Rain Water II (3D)                | Min-Heap Boundary Priority Queue  | O(MNlogMN| O(MN)    |
 | 83  | Maximum Average Pass Ratio                 | Max-Heap Marginal Gain Extraction | O(E logN)| O(N)     |
 | 84  | Total Cost to Hire K Workers               | Dual Min-Heaps Two-Ended Window   | O(K logC)| O(C)     |
 | 85  | Take Gifts From the Richest Pile           | Max-Heap Square Root Simulation   | O(K logN)| O(N)     |
 | 86  | Hand of Straights                          | Ordered Map Consecutive Grouping  | O(N logN)| O(N)     |
 | 87  | Divide Array in Sets of K Consecutive      | Sorted Frequency Hash Map Greed   | O(N logN)| O(N)     |
 | 88  | Valid Arrangement of Pairs                 | Hierholzer's Directed Euler Path  | O(V + E) | O(V + E) |
 | 89  | Patching Array                             | Greedy Reachable Range Doubling   | O(M+logN)| O(1)     |
 | 90  | Wiggle Subsequence                         | Greedy Alternating Extremes Peak  | O(N)     | O(1)     |
 | 91  | Monotone Increasing Digits                 | Reverse Inversion Scan + Suffix 9 | O(D)     | O(D)     |
 | 92  | Increasing Triplet Subsequence             | Two-Threshold Greedy Smallest     | O(N)     | O(1)     |
 | 93  | Maximum Length of Pair Chain               | Interval Scheduling by End Time   | O(N logN)| O(1)     |
 | 94  | Russian Doll Envelopes                     | Width Sort + Patience LIS         | O(N logN)| O(N)     |
 | 95  | Shortest Unsorted Continuous Subarray      | Two Pointers Running Min/Max Pass | O(N)     | O(1)     |
 | 96  | Huffman Encoding                           | Min-Heap Tree Merge + DFS Codes   | O(N logN)| O(N)     |
 | 97  | Fractional Knapsack                        | Value-to-Weight Ratio Descending  | O(N logN)| O(1)     |
 | 98  | Page Faults in LRU                         | Doubly-Linked Recency Hash Set    | O(N * C) | O(C)     |
 | 99  | Minimum Number of Coins                    | Canonical Denomination Greedy     | O(D)     | O(1)     |
 | 100 | Egyptian Fraction                          | Fibonacci-Sylvester Ceiling Greed | O(Terms) | O(1)     |
 ====================================================================================================
*/

// =========================================================
// 81. MINIMUM REFUELING STOPS
// =========================================================
...
// =========================================================
// 84. TOTAL COST TO HIRE K WORKERS
// =========================================================
...
// =========================================================
// 85. TAKE GIFTS FROM THE RICHEST PILE
// =========================================================
...
// Interview Explanation:
// - Problem Statement: In each of k seconds, replace the largest gift pile with floor(sqrt(pile)). Return remaining sum.
// - Approach: Max-Heap Simulation.
// - Intuition:
//   * To minimize final gift count, always reduce the maximum available pile.
//   * A max-heap retrieves and updates the largest pile in O(log N) per second.
//   * Extract top, compute floor(sqrt(top)), and reinsert.
//   * Sum remaining elements after k seconds.
// - Complexity: Time: O(N + K log N), Space: O(N).


// =========================================================
// 86. HAND OF STRAIGHTS
// =========================================================
...
// =========================================================
// 87. DIVIDE ARRAY IN SETS OF K CONSECUTIVE NUMBERS
// =========================================================
...
// Interview Explanation:
// - Problem Statement: Divide array into sets of k consecutive numbers.
// - Approach: Sorted Array Scan with Hash Map Frequency Deductions.
// - Intuition:
//   * Sort nums ascending.
//   * If current number x has not been consumed yet (cnt[x] > 0), x must serve as the start of a consecutive group.
//   * Greedily decrement frequencies for x, x + 1, ..., x + k - 1.
//   * If any required consecutive number has 0 frequency, return false.
// - Complexity: Time: O(N log N), Space: O(N).


// =========================================================
// 88. VALID ARRANGEMENT OF PAIRS
// =========================================================
...
// =========================================================
// 92. INCREASING TRIPLET SUBSEQUENCE
// =========================================================
...
// =========================================================
// 93. MAXIMUM LENGTH OF PAIR CHAIN
// =========================================================
...
// =========================================================
// 95. SHORTEST UNSORTED CONTINUOUS SUBARRAY
// =========================================================
...
// Interview Explanation:
// - Problem Statement: Find page faults in memory under Least Recently Used (LRU) caching with capacity c.
// - Approach: Doubly-Linked List + Hash Set Recency Tracking.
// - Intuition:
//   * If page is already present in cache: cache hit; move page to the front (most recent).
//   * If page is absent: cache miss / page fault.
//   * If cache is at full capacity, evict the least recently used page from the back of the list.
//   * Insert the new page at the front.
// - Complexity: Time: O(N * C), Space: O(C).


// =========================================================
// 99. MINIMUM NUMBER OF COINS
// =========================================================

vector<int> minPartitionCoins(int n) {
    const int coins[] = {2000, 500, 200, 100, 50, 20, 10, 5, 2, 1};
    vector<int> ans;
...
// Interview Explanation:
// - Problem Statement: Find minimum Indian currency notes/coins to make change for amount n.
// - Approach: Canonical Currency Greedy Backward Deduction.
// - Intuition:
//   * The standard Indian currency system satisfies the matroid greedy choice property.
//   * Larger denominations are always optimal combinations of smaller denominations.
//   * Greedily deduct the largest possible denomination repeatedly until n reaches 0.
// - Complexity: Time: O(N / 2000 + Denominations) = O(D), Space: O(1) auxiliary space.


// =========================================================
// 100. EGYPTIAN FRACTION
// =========================================================

vector<long long> egyptianFraction(long long nr, long long dr) {
    vector<long long> ans;

    while (nr != 0) {
        // ceil(dr / nr)
        long long x = (dr + nr - 1) / nr;
        ans.push_back(x);

        // nr/dr - 1/x = (nr * x - dr) / (dr * x)
        long long nextNr = nr * x - dr;
        long long nextDr = dr * x;

        long long g = std::gcd(nextNr, nextDr);
        nr = nextNr / g;
        dr = nextDr / g;
    }
    return ans;
...
// Interview Explanation:
// - Problem Statement: Represent fraction nr/dr as sum of distinct unit fractions (1/x).
// - Approach: Greedy Fibonacci-Sylvester Ceiling Algorithm.
// - Intuition:
//   * Greedily pick the largest possible unit fraction 1/x that does not exceed nr/dr.
//   * The denominator is x = ceil(dr / nr) = (dr + nr - 1) / nr.
//   * Subtract 1/x from nr/dr: newNr = nr*x - dr, newDr = dr*x.
//   * Reduce using greatest common divisor (gcd) to prevent overflow and repeat until nr becomes 0.
// - Complexity: Time: O(Terms generated), Space: O(1) auxiliary space.
