...
int kthElement(const vi &nums1, const vi &nums2, int k) {
    ...
    int m = nums1.size(), n = nums2.size(), l = max(0, k - n), r = min(k, m);
    while (l <= r) {
        int i = l + (r - l) / 2, j = k - i;
...
int median(const vi &nums1, const vi &nums2) {
    ...
    if (total % 2 == 1) return kthElement(nums1, nums2, total / 2 + 1);
    return (kthElement(nums1, nums2, total / 2) + kthElement(nums1, nums2, total / 2 + 1)) / 2;
}
...
int lengthOfLongestSubstring(const string &s) {
    ...
    for (int r = 0; r < (int)s.size(); r++) {
        if (last_seen.count(s[r]) && last_seen[s[r]] >= l) l = last_seen[s[r]] + 1;
        last_seen[s[r]] = r;
...
ll minMoves(const vi &nums, int k) {
    ...
    for (int i = 0; i < m; i++) { shifted[i] = pos[i] - i; pref[i + 1] = pref[i] + shifted[i]; }
    ll ans = 1e18;
    for (int l = 0; l + k <= m; l++) {
...
ll countSubsequences(const vi &nums, int k) {
    vl dp(k + 1, 0); dp[0] = 1;
    for (int x : nums)
        for (int sum = k; sum >= x; sum--) dp[sum] += dp[sum - x];
    return dp[k];
}
// Interview Explanation:
...
int maximumScore(const vi &nums, int k) {
    ...
    for (int i = 0; i <= n; i++) {
        while (!st.empty() && (i == n || nums[st.top()] >= nums[i])) {
            int mid = st.top(); st.pop();
            int l = st.empty() ? -1 : st.top(), r = i;
            if (l < k && k < r) ans = max(ans, 1LL * nums[mid] * (r - l - 1));
        }
        if (i < n) st.push(i);
    }
    return ans;
...
vi maxOfMins(const vi &arr) {
    ...
    vi res(n);
    for (int len = 1; len <= n; len++) res[len - 1] = ans[len];
    return res;
}
// Interview Explanation:
...
struct DigitDP {
    ...
    ll dp(int pos, bool started, bool tight, int state) {
        ...
        ll ans = !started ? dp(pos + 1, false, tight && (0 == limit), state) : 0;
...
vi findNegativeCycle(int n, const vector<EdgeItem> &edges) {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (const auto &e : edges) {
            if (dist[e.u] + e.w < dist[e.v]) { dist[e.v] = dist[e.u] + e.w; parent[e.v] = e.u; last_relaxed = e.v; }
        }
    }
...
ll makeArrayNonDecreasing(const vi &nums) {
    ...
    for (int x : nums) {
        max_heap.push(x);
        if (max_heap.top() > x) {
            total_cost += max_heap.top() - x; max_heap.pop(); max_heap.push(x);
        }
    }
    return total_cost;
}
