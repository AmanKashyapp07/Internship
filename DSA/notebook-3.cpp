...
=============================================================================*/
vector<int> getNodesInCycles(int n, const vector<vector<int>> &graph, vector<int> &indegree)
{
    queue<int> q;
...
bool dfs_bipartite(int u, int color, vector<int> &colors, const vector<vector<int>> &graph)
{
    colors[u] = color;
    for (int v : graph[u])
    {
        if (colors[v] == -1)
        {
            if (!dfs_bipartite(v, 1 - color, colors, graph))
                return false;
        }
...
vector<int> shortestPathDAG(int n, vector<vector<pair<int, int>>> &graph, int src)
{
    vector<int> indeg(n);
...
vector<int> dijkstra(int n, const vector<vector<pair<int, int>>> &graph, int src)
{
    vector<int> dist(n, INT_MAX);
    dist[src] = 0;
...
vector<int> shortestPathUnweighted(int n, const vector<vector<int>> &graph, int src)
{
    vector<int> dist(n, INT_MAX);
    dist[src] = 0;
    queue<int> q;
...
vector<int> lexicographical_topo_sort(int n, const vector<vector<int>> &graph, vector<int> indegree)
{
...
{
    bool isValid(int node, int color, const vector<vector<int>> &graph, const vector<int> &colors)
    {
    ...
    void dfs(int node, int m, const vector<vector<int>> &graph, vector<int> &colors, int &count)
    {
        if (node == graph.size())
        {
            count++;
        ...
        for (int color = 1; color <= m; color++)
        {
            ...
            {
                colors[node] = color;
                dfs(node + 1, m, graph, colors, count);
                colors[node] = 0;
            }
        }
    }
    int countColorings(int n, int m, const vector<vector<int>> &graph)
    {
        vector<int> colors(n, 0);
        int count = 0;
        dfs(0, m, graph, colors, count);
        return count;
    }
};
...
int shortestCycleInUndirectedGraph(int n, const vector<vector<int>> &graph)
{
...
} // Use binary splitting when an item can be chosen up to k times (bounded knapsack), where k can be large, making it inefficient to process each copy individually.

// Binary splitting decomposes k copies into groups of sizes 1, 2, 4, ..., remaining, converting each group into a 0/1 item. This reduces the number of items from O(k) to O(log k), allowing the problem to be solved efficiently using the standard 0/1 knapsack DP.

int knapsack_01(int n, int W, const vector<int> &weights, const vector<int> &values)
{
    vector<int> dp(W + 1, 0);
    for (int i = 0; i < n; i++)
    {
    ...
    return dp[W];
} // returns the maximum value that can be obtained with a weight limit W, given n items with specified weights and values. The time complexity is O(n * W) and space complexity is O(W).

/**
 * ============================================================================
 *                    BIPARTITE GRAPH & MATCHING CHEAT SHEET
 * ============================================================================
 *
 * 1. IDENTIFICATION PATTERNS & GRAPH REDUCTIONS
 * ----------------------------------------------------------------------------
 * A graph is bipartite if its vertices can be divided into two disjoint sets,
 * Left (L) and Right (R), such that every edge connects a vertex in L to one in R.
 *
 * - 2-Colorability: Can the graph be colored with 2 colors such that no two
 *   graphacent vertices share the same color?
 * - Odd Cycles: A graph is bipartite if and only if it contains NO odd cycles.
 * - Set Disjointness: Explicit assignment setups (Workers <-> Jobs,
 *   Students <-> Projects, Boys <-> Girls).
 * - Grid Coordinates: 2D grids where interactions only occur between graphacent
 *   cells can be partitioned using parity: (row + col) % 2.
 *   Even cells = Left, Odd cells = Right. (e.g., Domino placement, Knight moves).
 * - DAG Minimum Path Cover: To find the minimum number of vertex-disjoint paths
 *   to cover a Directed Acyclic Graph (DAG):
 *   1. Create a bipartite graph with L = N vertices and R = N vertices.
 *   2. For every directed edge u -> v in the DAG, add a matching edge u(L) -> v(R).
 *   3. Min Path Cover = (Total DAG Vertices) - (Maximum Bipartite Matching).
 *
 * /*
==============================================================================
...
- Natural partitions: Workers-Jobs, Students-Projects, Boys-Girls.
- Grid graphs: Left = (r+c)%2==0, Right = (r+c)%2==1.

DAG -> Bipartite Reduction (Minimum Path Cover):
- Create L = N, R = N.
- For every edge u -> v, add u(L) -> v(R).
- Min Path Cover = N - Maximum Matching.

------------------------------------------------------------------------------
2. CORE THEOREMS (Bipartite Graph)
----------------------------------
Let:
MM  = Maximum Matching
MVC = Minimum Vertex Cover
MIS = Maximum Independent Set
MEC = Minimum Edge Cover
...
- Minimum Edge Cover (No Isolated Vertices):
    MEC = V - MM

- Perfect Matching:
    Exists iff MM = |L| = |R| (balanced graph).

- Hall's Marriage Theorem:
    Perfect matching exists iff
    |S| <= |N(S)| for every subset S of L.

------------------------------------------------------------------------------
3. MINIMUM VERTEX COVER (After Maximum Matching)
------------------------------------------------
1. Start DFS/BFS from all unmatched Left vertices.
2. Traverse:
      L -> R via UNMATCHED edges.
      R -> L via MATCHED edges.
3. Let Vis = visited vertices.
4. Minimum Vertex Cover =
      (Unvisited Left) U (Visited Right)

------------------------------------------------------------------------------
4. ALGORITHMS
-------------
- Kuhn (DFS Augmenting Paths): O(VE)
...
- MVC = MM
- MIS = V - MM
- MEC = V - MM (no isolated vertices)
- Min Path Cover (DAG) = V - MM
- Perfect Matching: MM = |L| = |R| + Hall's condition
...
int LCSLengthOf2Permutations(vector<int> &A, vector<int> &B)
{
...
vector<int> LCSof2Permutations(vector<int> &A, vector<int> &B)
{
    unordered_map<int, int> pos;
    ...
    vector<int> val; // corresponding values
...
// main theory : if we have to find LCS of two permutations of size n and size m , if we map the elements of one permutation to their indices in the other, we can reduce the problem to finding the Longest Increasing Subsequence (LIS) of the mapped indices. The length of this LIS will give us the length of the LCS of the two permutations. The time complexity is O(n log n) due to the LIS computation using binary search. Proof - When we map the elements of one permutation to their indices in the other, we create a sequence of indices that represents the order of elements in the second permutation as they appear in the first. The longest increasing subsequence of this index sequence corresponds to the longest common subsequence of the two original permutations, as it maintains the relative order of elements.
...
int lis(vector<int> &arr)
{
    ...
    for (int x : arr)
    {
        auto it = lower_bound(tail.begin(), tail.end(), x); // find the first element in tail which is >= x
        if (it == tail.end())
            tail.push_back(x);
        else
            *it = x;
...
vector<int> lexicographicalMinimumTopologicalOrder(vector<vector<int>> &g) {
    int n = g.size();
    ...
    return topo;
} // using priorirty queue ensures we get the lexicographically smallest topological order. The time complexity is O(V + E log V) due to the priority queue operations, and the space complexity is O(V + E) for storing the graph and indegree array.
vector<int> topo_sort(int n, vector<vector<int>> &g) {
    ...
    return topo;
} // This function performs a topological sort on a directed graph represented as an graphacency list. It uses Kahn's algorithm, which is based on tracking the in-degrees of vertices. The time complexity is O(V + E) and the space complexity is O(V + E).

vector<int> possible_lengths(int n, vector<vector<int>> &graph) {

    vector<int> topo = topo_sort(n, graph);
