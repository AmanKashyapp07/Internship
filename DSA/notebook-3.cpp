...
vector<int> buildCycle(int start, const vector<int> &parent) {
    ...
    while (true) {
        cycle.push_back(cur); cur = parent[cur];
        if (cur == start) break;
    }
    return cycle;
}
...
bool dfs_bipartite(int u, int color, vector<int> &colors, const vector<vector<int>> &graph) {
    ...
    for (int v : graph[u]) {
        if (colors[v] == -1) {
            if (!dfs_bipartite(v, 1 - color, colors, graph)) return false;
...
vi shortestPathDAG(int n, vector<vector<pair<int, int>>> &graph, int src) {
    vi indeg(n);
    for (int u = 0; u < n; u++) for (auto [v, wt] : graph[u]) indeg[v]++;
...
int CountSubsetsWithSumK(const vi &nums, int k) {
    vi dp(k + 1, 0); dp[0] = 1;
    for (int num : nums) {
        for (int j = k; j >= num; j--) dp[j] += dp[j - num];
    }
    return dp[k];
}
...
int minSubsetSumDifference(const vi &nums) {
    ...
    for (int num : nums) {
        for (int j = totalSum / 2; j >= num; j--) dp[j] = dp[j] || dp[j - num];
    }
...
vi getMoneySums(vi &coins) {
    ...
    for (int coin : coins) {
        for (int sum = total; sum >= coin; sum--) if (dp[sum - coin]) dp[sum] = true;
    }
    vi possibleSums;
...
vi bellman_ford(int n, const vector<Edge> &edges, int src) {
    ...
    for (int i = 1; i <= n - 1; i++) {
        for (const auto &e : edges) {
            if (dist[e.u] != LLONG_MAX && dist[e.u] + e.w < dist[e.v]) dist[e.v] = dist[e.u] + e.w;
        }
    }
    for (const auto &e : edges) {
...
void DPonDAG(int n, const vector<vector<pair<int, int>>> &graph, int src) {
    vi indeg(n, 0);
    for (int u = 0; u < n; u++) for (auto [v, wt] : graph[u]) indeg[v]++;
    vi topo_order; queue<int> q;
    for (int i = 0; i < n; i++) if (indeg[i] == 0) q.push(i);
    while (!q.empty()) {
        int u = q.front(); q.pop(); topo_order.push_back(u);
        for (auto [v, wt] : graph[u]) if (--indeg[v] == 0) q.push(v);
    }
    vi dist(n, INT_MAX); dist[src] = 0;
    for (auto u : topo_order) {
        if (dist[u] != INT_MAX) {
            for (auto [v, wt] : graph[u]) dist[v] = min((long long)dist[v], (long long)dist[u] + wt);
        }
    }
}
...
int findCheapestPrice(int n, vector<vector<int>> &flights, int src, int dst, int k) {
    ...
    int maxFlights = k + 1;
    vvi dist(n, vi(maxFlights + 1, INT_MAX));
...
int shortestPathVisitingAllNodes(int n, const vvi &graph) {
    int allVisitedMask = (1 << n) - 1; queue<pair<int, int>> q;
    vvi dist(n, vi(1 << n, INT_MAX));
    ...
    while (!q.empty()) {
        ...
        for (int neighbor : graph[node]) {
            ...
            if (dist[neighbor][nextMask] > currentDist + 1) {
                dist[neighbor][nextMask] = currentDist + 1; q.push({neighbor, nextMask});
            }
        }
    }
    return -1;
