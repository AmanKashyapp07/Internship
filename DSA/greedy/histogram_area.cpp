 ...
 * 
 * Logic/Approach:
 * Monotonic stack storing indices of increasing heights.
 */
...
ll largestHistogram(vector<int>& h) {
    ...
    for (int i = 0; i <= m; i++) {
        ...
        while (!st.empty() && h[st.top()] >= curHeight) {
            ...
            best = max(best, 1LL * height * width); // for max square area , best = max(best, 1LL * min(height,width) * min(height,width));
        }
