...
bool subsetSumMemoHelper(int idx, int target, const vector<int> &nums,
...
int countSubsetsMemoHelper(int idx, int target, const vector<int> &nums,
                           vector<vector<int>> &dp) {
  if (idx == nums.size())
    return target == 0 ? 1 : 0;
  if (dp[idx][target] != -1)
...
int main() {
  ...
  cout << "All clear Subset Sum Memoization and Tabulation tests passed "
          "successfully!"
       << endl;
  return 0;
}