...
bool subsetSumMemoHelper(int idx, int target, const vector<int> &arr,
...
int countSubsetsMemoHelper(int idx, int target, const vector<int> &arr,
                           vector<vector<int>> &dp) {
  if (idx == arr.size())
    return target == 0 ? 1 : 0;
  if (dp[idx][target] != -1)
...
int knapsackMemoHelper(int idx, int W, const vector<int> &wt,
                       const vector<int> &val, vector<vector<int>> &dp) {
  if (idx == wt.size())
    return 0;
  if (dp[idx][W] != -1)
    return dp[idx][W];
...
int minCoinsMemoHelper(int idx, int amount, const vector<int> &coins,
                       vector<vector<int>> &dp) {
  if (idx == coins.size())
  ...
  int take = 1e9;
  if (coins[idx] <= amount) {
    take = 1 + minCoinsMemoHelper(idx, amount - coins[idx], coins, dp);
  }
  return dp[idx][amount] = min(take, notTake);
}

int waysToMakeSumMemoHelper(int idx, int amount, const vector<int> &coins,
                            vector<vector<int>> &dp) {
  if (idx == coins.size())
    return amount == 0 ? 1 : 0;
  if (dp[idx][amount] != -1)
  ...
  int take = 0;
  if (coins[idx] <= amount) {
    take = waysToMakeSumMemoHelper(idx, amount - coins[idx], coins, dp);
  }
...
int min_subset_sum_difference_memo(const vector<int> &arr) {
  ...
  subsetSumMemoHelper(0, k, arr, dp);
...
int main() {
  ...
  int max_W = 7;
  assert(knapsack_memo(weights, values, max_W) == 9);
  assert(knapsack_tab(weights, values, max_W) == 9);

  vector<int> coins = {1, 2, 3};
  int amount = 5;
  assert(min_coins_memo(coins, amount) == 2);
  assert(min_coins_tab(coins, amount) == 2);
  assert(ways_to_make_sum_memo(coins, amount) == 5);
  assert(ways_to_make_sum_tab(coins, amount) == 5);

  vector<int> arr = {1, 6, 11, 5};
  assert(min_subset_sum_difference_memo(arr) == 1);
  assert(min_subset_sum_difference_tab(arr) == 1);

  cout << "All Tier-1 Knapsack and Unbounded variants compiled and verified "
          "successfully!"
       << endl;
  return 0;
}