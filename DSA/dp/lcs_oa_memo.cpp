...
int lcsHelper(int i, int j, const string &s1, const string &s2,
  ...
  return dp[i][j] = max(lcsHelper(i - 1, j, s1, s2, dp),
...
string getLCS(string text1, string text2) {
  ...
  while (i > 0 && j > 0) {
    if (text1[i - 1] == text2[j - 1]) {
      ...
      // Safely query helper values since top-down might leave some adjacent
      // states unvisited
...
int substringHelper(int i, int j, const string &s1, const string &s2,
  ...
  // Ensure all subproblems are evaluated to accurately track the global max
  // length
...
string shortestCommonSupersequence(string str1, string str2) {
  ...
  while (i > 0 && j > 0) {
    if (str1[i - 1] == str2[j - 1]) {
      ...
      if (lcsHelper(i - 1, j, str1, str2, dp) >
          lcsHelper(i, j - 1, str1, str2, dp)) {
        scs.push_back(str1[i - 1]);
        i--;
      } else {
...
int repeatingSubseqHelper(int i, int j, const string &s,
                          vector<vector<int>> &dp) {
  if (i == 0 || j == 0)
    return 0;
  if (dp[i][j] != -1)
  ...
  return dp[i][j] = max(repeatingSubseqHelper(i - 1, j, s, dp),
                        repeatingSubseqHelper(i, j - 1, s, dp));
}
...
int editDistanceHelper(int i, int j, const string &s1, const string &s2,
                       vector<vector<int>> &dp) {
  if (i == 0)
    return j;
  if (j == 0)
...
int distinctSubseqHelper(int i, int j, const string &s, const string &t,
                         vector<vector<int>> &dp) {
  if (j == 0)
    return 1;
  if (i == 0)
  ...
  if (s[i - 1] == t[j - 1]) {
    return dp[i][j] = distinctSubseqHelper(i - 1, j - 1, s, t, dp) +
                      distinctSubseqHelper(i - 1, j, s, t, dp);
  }
