 ...
 * 
 * Description:
 * Count the number of reachable nodes for each node in a directed acyclic graph.
 * 
 * Logic/Approach:
 * Topological sort + bitset representation. Process nodes in reverse topological order, taking the union of child bitsets.
 */
...
signed main()
{
    ...
    // reachable[i] is a vector of bits where the j-th bit is set if node i can reach node j. The count() function of bitset returns the number of bits that are set to 1, which in this context gives us the number of nodes that are reachable from node i.
    // 3. Output the result
    for (int i = 0; i < n; i++)
    {
...
// we can do this also without using bitset and using vector of sets but that would be less efficient in terms of both time and space complexity, because we would have to store the reachable nodes in a set for each node, which can lead to higher memory usage and slower union operations compared to using bitsets, and also LOC would be more.