...
```
DOCUMENT CORPUS
Doc 1: "Docker container sandboxes in MagnusCI"
Doc 2: "Docker Engine API for NexusIDE workspaces"
...
|  "magnusci"      --->   [ (Doc 1, 1, pos: 4) ]                                    |
|  "nexuside"      --->   [ (Doc 2, 1, pos: 4) ]                                    |
...
## 3. How I Used It (Search & Developer Tooling)

- **Instant Full-Text Search Integration:**
  - Integrated Meilisearch to index workspace files, symbols, functions, and pipeline build step logs.
  - Configured custom **Ranking Rules** (`words`, `typo`, `proximity`, `attribute`, `exactness`) to prioritize exact symbol matches over partial text matches.
  - Used **Tenant Tokens** to securely scope search queries so users could search only within their own private repositories and projects.
...
## 5. Meilisearch vs. Elasticsearch vs. PostgreSQL Full-Text Search
